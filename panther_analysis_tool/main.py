...
def release(
    ...
    ignore_files: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_IGNORE_FILES", help="List of files to ignore"
    ),
    kms_key: Optional[str] = typer.Option(
        None, envvar="PANTHER_KMS_KEY", help="AWS KMS key ID to sign the release"
    ),
    min_test: Optional[bool] = typer.Option(False, help="Run minimal tests"),
    out: Optional[Path] = typer.Option(
        Path("panther-analysis-all.zip"), envvar="PANTHER_OUT", help="Output zip file path"
    ),
    path: Optional[Path] = typer.Option(Path("."), help="Path to the repo"),
    skip_test: Optional[bool] = typer.Option(
        False, envvar="PANTHER_SKIP_TEST", help="Skip all tests"
    ),
    skip_disabled_tests: Optional[bool] = typer.Option(
        False, envvar="PANTHER_SKIP_DISABLED_TESTS", help="Skip disabled tests"
    ),
    available_destination: Optional[str] = typer.Option(
        None, envvar="PANTHER_AVAILABLE_DESTINATION", help="Optional S3 destination"
    ),
...
def test(
    ...
    filter: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_FILTER", metavar="KEY=VALUE", help="Filter detections by key=value"
    ),
    minimum_tests: int = typer.Option(
        0, envvar="PANTHER_MINIMUM_TESTS", help="Minimum number of tests to pass a detection"
    ),
    ...
    ignore_files: List[str] = typer.Option(
        [], envvar="PANTHER_IGNORE_FILES", help="Files to ignore", show_default=False
    ),
    skip_disabled_tests: bool = typer.Option(
        False, envvar="PANTHER_SKIP_DISABLED_TESTS", help="Skip disabled tests"
    ),
    available_destination: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_AVAILABLE_DESTINATION", help="Available destination(s)"
    ),
    ...
    valid_table_names: List[str] = typer.Option(
        [], help="Additional valid table names", show_default=False
    ),
    test_names: Optional[List[str]] = typer.Option(
        None,
        envvar="PANTHER_TEST_NAMES",
        metavar="TEST_NAME",
        help="Run only tests with these names",
    ),
):
...
def publish_command(
    ...
    # GitHub args
    github_tag: str = typer.Option(
        ..., envvar="PANTHER_GITHUB_TAG", help="The tag name for this release"
    ),
    github_branch: str = typer.Option(
        "main", envvar="PANTHER_GITHUB_BRANCH", help="The branch to base the release on"
    ),
    github_owner: str = typer.Option(
        "panther-labs", envvar="PANTHER_GITHUB_OWNER", help="GitHub owner of the repository"
    ),
    github_repository: str = typer.Option(
        "panther-analysis", envvar="PANTHER_GITHUB_REPOSITORY", help="GitHub repository name"
    ),
    body: str = typer.Option("", envvar="PANTHER_BODY", help="Release text body"),
    # Standard shared args
    filter: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_FILTER", metavar="KEY=VALUE", help="Filter detections"
    ),
    kms_key: Optional[str] = typer.Option(
        None, envvar="PANTHER_KMS_KEY", help="KMS key for signing the release"
    ),
    minimum_tests: int = typer.Option(
        0, envvar="PANTHER_MINIMUM_TESTS", help="Minimum number of tests required"
    ),
    ...
    available_destination: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_AVAILABLE_DESTINATION", help="Upload destination(s)"
    ),
    ignore_files: List[str] = typer.Option(
        [], envvar="PANTHER_IGNORE_FILES", help="Files to ignore", show_default=False
    ),
...
def upload(
    ...
    available_destination: Optional[List[str]] = typer.Option(
        None, envvar="PANTHER_AVAILABLE_DESTINATION", help="Upload destinations"
    ),
    sort_test_results: bool = typer.Option(False, help="Sort test results"),
    show_failures_only: bool = typer.Option(False, help="Only show failed test results"),
    print_failed_test_results_only: bool = typer.Option(
        False, help="Only print failed test results"
    ),
    ignore_table_names: bool = typer.Option(False, help="Ignore table name validation"),
    valid_table_names: List[str] = typer.Option(
        [], help="Additional valid tables", show_default=False
    ),
):
    if ignore_extra_keys:
        RULE_SCHEMA._ignore_extra_keys = True  # pylint: disable=protected-access
...
def test_lookup_table_cmd(
    ...
    path: str = typer.Option(
        ..., envvar="PANTHER_PATH", help="The relative path to a lookup table input file."
    ),
):
...
def benchmark_command(
    ...
    path: str = typer.Option(
        ".", envvar="PANTHER_PATH", help="The relative path to Panther policies and rules."
    ),
    out: str = typer.Option(".", envvar="PANTHER_OUT", help="The path to store output files."),
