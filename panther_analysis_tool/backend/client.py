...
class BulkUploadResponse:  # pylint: disable=too-many-instance-attributes
    ...
    skills: BulkUploadStatistics = field(
        default_factory=lambda: BulkUploadStatistics(total=0, new=0, modified=0)
    )
...
def to_bulk_upload_response(data: Any) -> BackendResponse[BulkUploadResponse]:
    ...
    return BackendResponse(
        ...
        data=BulkUploadResponse(
            ...
            data_models=BulkUploadStatistics(
                total=data.get("dataModels", default_stats).get("total"),
                new=data.get("dataModels", default_stats).get("new"),
                modified=data.get("dataModels", default_stats).get("modified"),
            ),
            lookup_tables=BulkUploadStatistics(
                total=data.get("lookupTables", default_stats).get("total"),
                new=data.get("lookupTables", default_stats).get("new"),
                modified=data.get("lookupTables", default_stats).get("modified"),
            ),
            global_helpers=BulkUploadStatistics(
                total=data.get("globalHelpers", default_stats).get("total"),
                new=data.get("globalHelpers", default_stats).get("new"),
                modified=data.get("globalHelpers", default_stats).get("modified"),
            ),
            correlation_rules=BulkUploadStatistics(
