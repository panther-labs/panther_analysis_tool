...
class AnalysisCache:
    ...
    def get_spec_for_version(self, analysis_id: int, base_version: int) -> Tuple[int, Optional[str]]:
        self.cursor.execute("SELECT id, spec FROM analysis_specs WHERE id_value = ? AND version = ?", (analysis_id, base_version))
        row = self.cursor.fetchone()
        if row is None:
            return None, None
        return row[0], row[1]
    
    def get_latest_spec(self, analysis_id: int) -> Tuple[Optional[int], Optional[str], Optional[int]]:
        self.cursor.execute("SELECT id, spec, version FROM analysis_specs WHERE id_value = ? ORDER BY version DESC LIMIT 1", (analysis_id,))
        row = self.cursor.fetchone()
        if row is None:
