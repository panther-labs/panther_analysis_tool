...
class GitManager:
    ...
    def panther_remote(self) -> str:
        ...
        panther_remote_output = subprocess.run(["git", "remote", "-v"], capture_output=True, text=True)
    ...
    def merge_base(self, branch: str) -> str:
        ...
        merge_base_output = subprocess.run(["git", "merge-base", panther_commit, branch], capture_output=True, text=True)
    ...
    def git_root(self) -> str:
        ...
        self._git_root = subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, check=True).stdout.strip()
        return self._git_root
