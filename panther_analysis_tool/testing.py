...
class FunctionTestResult:
    ...
    def new(cls,
            ...
            raw_exception: Optional[Exception] = None) -> Optional['FunctionTestResult']:
...
class TestCaseEvaluator:
    ...
    def _get_result_status(self) -> bool:
        ...
        return self._detection_result.input_exception is None and \
               not matched and \
               not self._detection_result.detection_evaluation_failed
    ...
    def interpret(self) -> TestResult:
        ...
        function_results = dict(
            detectionFunction=FunctionTestResult.new(self._detection_result.matched, self._detection_result.detection_exception)
        )
        ...
        if (
            (self._spec.expectations.detection and self._detection_result.detection_type != TYPE_POLICY) or
            (not self._spec.expectations.detection and self._detection_result.detection_type == TYPE_POLICY)
        ):
            function_results.update(
                dict(
                    titleFunction=FunctionTestResult.new(self._detection_result.title_output, self._detection_result.title_exception),
                    descriptionFunction=FunctionTestResult.new(
                        self._detection_result.description_output, self._detection_result.description_exception
                    ),
                    referenceFunction=FunctionTestResult.new(
                        self._detection_result.reference_output, self._detection_result.reference_exception
                    ),
                    severityFunction=FunctionTestResult.new(
                        self._detection_result.severity_output, self._detection_result.severity_exception
                    ),
                    runbookFunction=FunctionTestResult.new(self._detection_result.runbook_output, self._detection_result.runbook_exception),
                    destinationsFunction=FunctionTestResult.new(
                        self._detection_result.destinations_output, self._detection_result.destinations_exception
                    ),
                    dedupFunction=FunctionTestResult.new(self._detection_result.dedup_output, self._detection_result.dedup_exception),
                    alertContextFunction=FunctionTestResult.new(
                        self._detection_result.alert_context_output, self._detection_result.alert_context_exception
                    )
                )
            )
        ...
        return TestResult(
            ...
            genericError=FunctionTestResult.format_exception(generic_error, title=generic_error_title),
