...
from panther_analysis_tool.analysis_utils import LoadAnalysisSpecsResult, load_analysis_specs_ex, lookup_analysis_id
...
def clone_analysis(analysis_id: str, filter: Optional[List[str]], mutator: Callable[[Any], None]) -> None:
    ...
    existing = [spec for spec in load_analysis_specs_ex(["."], [], True) if CACHE_DIR not in spec.spec_filename]

    if analysis_id is not None:
        for spec in existing:
            if lookup_analysis_id(spec.analysis_spec) == analysis_id:
                # FIXME update the spec in place with the mutator
