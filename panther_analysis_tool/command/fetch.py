...
def import_from_github_branch() -> None:
    ...
    subprocess.run(["git", "clone", "-b", PANTHER_ANALYSIS_GITHUB_BRANCH, "https://github.com/panther-labs/panther-analysis.git", os.path.join(CACHE_DIR, "panther-analysis")])
...
def import_sqlite() -> None:
    ...
    for spec in all_specs:
        ...
        spec_id = cursor.execute("INSERT INTO analysis_specs (id_field, id_value, spec, file_path, version) VALUES (?, ?, ?, ?, ?);",
                    (id_field, id_value, spec.raw_file_content, str(relpath), spec_version)).lastrowid
