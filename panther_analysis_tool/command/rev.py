...
def rev_analysis(analysis_id: str) -> None:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, id_field, id_value, spec, file_path FROM analysis_specs WHERE UPPER(id_value) = ? ORDER BY version DESC LIMIT 1",
          (analysis_id.upper(),))
    ...
    new_spec_id = cursor.execute(
        ...
        (id_field, id_value, new_spec, file_path, spec_version)).lastrowid
