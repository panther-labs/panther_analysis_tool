...
def merge_analysis(analysis_id: Optional[str] = None, migrate: bool = False) -> Tuple[int, str]:
    ...
    for user_spec in all_specs:
        ...
        latest_base_spec_str, latest_file_content, latest_version = loader.load_latest_spec(user_spec)
...
class Loader:
    def __init__(self, cursor: sqlite3.Cursor, cache: analysis_cache.AnalysisCache, git_manager: git.GitManager, yaml: ruamel.yaml.YAML):
        self.cursor = cursor
        self.cache = cache
    ...
    def load_base_spec(self, spec: LoadAnalysisSpecsResult, use_git: bool) -> Tuple[Optional[str], Optional[bytes]]:
    ...
    def load_latest_spec(self, spec: LoadAnalysisSpecsResult) -> Tuple[Optional[str], Optional[bytes], Optional[int]]:
        analysis_id = lookup_analysis_id(spec.analysis_spec)
    ...
    def was_deleted_by_panther(self, filename: str) -> bool:
        ...
        proc = subprocess.run(
            ["git", "log", "--diff-filter=D", "--oneline", "-1", f"{merge_base}..{panther_commit}", "--", filename],
    ...
    def still_exists_in_panther(self, filename: str) -> Optional[bytes]:
        ...
        proc = subprocess.run(
            ["git", "show", f"{panther_commit}:{filename}"],
            check=False, capture_output=True)
...
def get_path_from_spec(spec: LoadAnalysisSpecsResult) -> str:
    ...
    if spec.analysis_spec.get("AnalysisType") == AnalysisTypes.POLICY:
        folder = "policies"
    if spec.analysis_spec.get("AnalysisType") in [AnalysisTypes.SAVED_QUERY, AnalysisTypes.SCHEDULED_QUERY]:
        folder = "queries"
