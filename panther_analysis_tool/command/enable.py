...
def run(analysis_id: Optional[str], **kwargs) -> Tuple[int, str]:
    ...
    return enable_analysis_id(analysis_id)

def enable_analysis_id(analysis_id: str) -> Tuple[int, str]:
    ...
    for managed_spec in managed_specs.detections:
        if analysis_id == get_spec_id(managed_spec.analysis_spec):
            user_spec = user_specs_by_id.get(analysis_id)
            enable_analysis_spec(managed_spec, user_spec)

    return 0, "Enabled"


def enable_analysis_filter(filters: List[str]) -> Tuple[int, str]:
    # get all analysis specs
    # First classify each file, always include globals and data models location
    all_specs, _ = load_analysis(".", True, [], [])
    if all_specs.empty():
    ...
    # Apply the filters as needed
    parsed_filters, parsed_filters_inverted = parse_filter(filters)
...
def enable_analysis_spec(managed_analysis_spec: ClassifiedAnalysis, user_analysis_spec: Optional[ClassifiedAnalysis]) -> None:
    if user_analysis_spec is None:
        ...
        match managed_analysis_spec.analysis_spec["AnalysisType"]:
            ...
            case _:
                raise ValueError(f"Unknown analysis type: {managed_analysis_spec.analysis_spec['AnalysisType']}")
        new_file_path.parent.mkdir(parents=True, exist_ok=True)
        with open(new_file_path, "w", encoding="utf-8") as spec_file:
            yaml.dump(new_spec, spec_file)
