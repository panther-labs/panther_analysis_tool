...
from panther_analysis_tool.backend.client import Client as BackendClient, MetricsParams, PerfTestParams
from panther_analysis_tool.constants import AnalysisTypes
from panther_analysis_tool.util import log_and_write_to_file
from panther_analysis_tool.zip_chunker import ZipArgs, analysis_for_chunks, chunk_analysis
...
class PerformanceTestIteration:
    ...
    def __str__(self):
        return f"Read time (nanoseconds): {self.read_time_nanos}\n" \
...
def run(backend: BackendClient, args: argparse.Namespace) -> Tuple[int, str]:
    ...
    params = PerfTestParams(zip_bytes=buffer.read(), log_type=log_type, hour=hour_or_err, timeout=timeout.astimezone())
    iterations = []
    logged = False
...
def validate_rule_count(analyses: List[ClassifiedAnalysis]) -> (Union[ClassifiedAnalysis, str]):
    if len(analyses) != 1:
        return f"Only 1 detection must be specified for benchmarking, {len(analyses)} were specified:" \
            f" {[a.file_name for a in analyses]}"
    analysis = analyses[0]
    if analysis.analysis_spec["AnalysisType"] != AnalysisTypes.RULE:
        return f"Only rules are supported for performance testing, but {analysis.analysis_spec['AnalysisType']}" \
                  f" was provided"
    return analysis
...
def validate_log_type(args: argparse.Namespace, rule: ClassifiedAnalysis) -> (str, Optional[str]):
    ...
    if log_type is None:
        if len(rule_log_types) > 1:
            return log_type, f"Multiple log types specified for {rule.file_name}: {rule_log_types}, please use the" \
                      f" --log-type arg to specify one."
        log_type = rule_log_types[0]
    else:
        if not str(log_type).casefold() in map(str.casefold, rule_log_types):
            return log_type, f"Provided log type {log_type} was not found in log types for {rule.file_name}:" \
                      f" {rule_log_types}"
    return log_type, None


def validate_hour(args: argparse.Namespace, log_type: str, backend: BackendClient) -> Union[datetime.datetime, str]:
    hour = getattr(args, "hour", None)
    now_truncated = datetime.datetime.now()\
        .replace(minute=0, second=0, microsecond=0).astimezone() + datetime.timedelta(microseconds=-1)
    window_begin = now_truncated + datetime.timedelta(weeks=-2, microseconds=1)
    if hour is None:
        end_time = now_truncated
        start_time = window_begin
        metrics_response = backend.get_metrics(MetricsParams(
            from_date=start_time,
            to_date=end_time,
            interval_in_minutes=60,
        ))
        data_for_log_type = next((x for x in metrics_response.data.bytes_processed_per_source
                                  if x.label.casefold() == log_type.casefold()), None)
        if data_for_log_type is None:
            return f"No data found on Panther for log_type {log_type} in past two weeks. This can occur if" \
                      f" ingestion began within the last 24 hours."
        max_data_hour = max(data_for_log_type.breakdown, key=data_for_log_type.breakdown.get, default=None)
        ...
        logging.info(f"To re-run with this same data please add '--hour {max_data_hour}' to the commandline invocation")
    else:
        ...
        data_for_log_type = next((x for x in metrics_response.data.bytes_processed_per_source
                                  if x.label.casefold() == log_type.casefold()), None)
        if data_for_log_type is None:
            return f"No data found on Panther for log_type {log_type} at specified hour: {hour.isoformat()}. Please" \
        ...
        if selected_hour is None or data_for_log_type.breakdown[selected_hour] == 0:
            return f"No data processed on Panther for log_type {log_type} at specified hour: {hour.isoformat()}." \
