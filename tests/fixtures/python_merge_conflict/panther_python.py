...
def rule(event: Any) -> bool:
    ...
    old = event.deep_get("details", "old_value", default="")  # panther comment
    return something(event, old, new)


def something(event: Any, old: str, new: str) -> bool:
    return all(
        [
...
def alert_context(event: Any) -> dict[str, Any]:
    return asana_alert_context(event)  # panther was here
