...
def rule(event: Any) -> bool:
    # customer comment
    new = event.deep_get("details", "new_value", default="")  # customer comment
...
def title(event: Any) -> str:
    ...
    return f"Asana user [{a_c.get('resource_name')}] was made an admin "


def alert_context(event: Any) -> dict[str, Any]:
    return asana_alert_context(event)  # customer was here
