...
class TestPantherCLI(TestCase):
    ...
    def test_valid_json_policy_spec(self):
        for spec_filename, _, loaded_spec in pat.load_policy_specs('tests/fixtures'):
    ...
    def test_load_policy_specs_from_folder(self):
        args = pat.setup_parser().parse_args('test --policies tests/fixtures'.split())
        return_code, invalid_specs = pat.test_policies(args)
    ...
    def test_zip_policies(self):
        ...
        return_code, out_filename = pat.zip_policies(args)
