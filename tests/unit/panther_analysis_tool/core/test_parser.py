...
from tests.unit.panther_analysis_tool.test_main import mock_test_analysis
    ...
    def test_parse_filters(self):
        with patch(
            ...
            result = runner.invoke(
                pat.app,
                f"test --path {DETECTIONS_FIXTURES_PATH}/valid_analysis --filter AnalysisType=policy,global --filter Severity=Critical --filter Enabled=true".split(),
            )
            ...
            parsed_filters, parsed_filters_inverted = parse.parse_filter(args.filter)
