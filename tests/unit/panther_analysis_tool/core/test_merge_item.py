...
def test_merge_item_write_merge_conflicts(
    ...
    mocker.patch(
        "panther_analysis_tool.core.merge_item.Dict.merge_dict",
...
def test_merge_file_yaml_conflict_solve(mocker: MockerFixture, tmp_path: pathlib.Path) -> None:
    ...
    mocker.patch(
        "panther_analysis_tool.gui.yaml_conflict_resolver_gui.YAMLConflictResolverApp",
...
def test_diff_dict_keys() -> None:
    dict1 = {"a": 1, "b": 2, "c": 3}
    dict2 = {"a": 1, "b": 3, "d": 4}
    diff_keys = merge_item.diff_dict_keys(dict1, dict2)
    assert diff_keys == ["b"]
...
def test_dict_diff() -> None:
    ...
    diff_items = merge_item.Dict(customer_dict=customer_dict).merge_dict(
        base_dict=base_dict, latest_dict=latest_dict
    )
    assert diff_items == [
        merge_item.DictMergeConflict("a", "customer", "latest", "base"),
    ]
...
def test_dict_diff_customer_deleted_key_auto_accept_yours_keeps_absent() -> None:
    ...
    merge_item.Dict(customer_dict=customer_dict, auto_accept=AutoAcceptOption.YOURS).merge_dict(
        base_dict=base_dict, latest_dict=latest_dict
    )
    assert "RemovedByCustomer" not in customer_dict
...
def test_dict_diff_customer_deleted_key_auto_accept_panthers_adds_key() -> None:
    ...
    customer_dict: dict = {}
    latest_dict = {"RemovedByCustomer": "latest_val"}
    merge_item.Dict(customer_dict=customer_dict, auto_accept=AutoAcceptOption.PANTHERS).merge_dict(
        base_dict=base_dict, latest_dict=latest_dict
    )
