...
class TestInitLogTypeCache(unittest.TestCase):
    ...
    def test_init_negative_caching(self):
        """Test that failed fetches are negatively cached (no repeated API calls)."""
        mock_backend = Mock()
        mock_backend.list_schemas.side_effect = Exception("Network error")

        # First call fails
        result1 = init_log_type_cache(mock_backend)
        self.assertFalse(result1)

        # Second call should not retry the API
        result2 = init_log_type_cache(mock_backend)
...
class TestSplitAnalysisByLogTypeSupport(unittest.TestCase):
    ...
    def test_no_cache_returns_all_supported_no_errors(self):
        """When backend fetch fails, all items pass through (no validation)."""
        items = [
...
class TestLogTypeCache(unittest.TestCase):
    ...
    def test_cache_initialized_flag(self):
        """Test that initialized flag tracks cache state."""
        self.assertFalse(LogTypeCache.is_initialized())

        LogTypeCache.set_log_types({"AWS.CloudTrail"})
        self.assertTrue(LogTypeCache.is_initialized())

        LogTypeCache.clear()
        self.assertFalse(LogTypeCache.is_initialized())

    def test_cache_initialize_once_negative_caching(self):
        """Test initialize_once marks as initialized even on failure (negative caching)."""
        self.assertFalse(LogTypeCache.is_initialized())

        result = LogTypeCache.initialize_once(lambda: None)
        self.assertFalse(result)
        self.assertTrue(LogTypeCache.is_initialized())
        self.assertIsNone(LogTypeCache.get_log_types())
    ...
    def test_cache_thread_safety_init(self):
        """Test that concurrent init_log_type_cache calls only fetch once."""
        import threading

        mock_backend = _make_mock_backend(["AWS.CloudTrail"])
        ...
        self.assertEqual(len(errors), 0)
        # Backend should only be called once despite concurrent init attempts
        self.assertEqual(mock_backend.list_schemas.call_count, 1)
